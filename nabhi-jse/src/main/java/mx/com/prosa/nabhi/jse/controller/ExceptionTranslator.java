...
public class ExceptionTranslator {
    ...
    public ResponseEntity < ResponsePayload < String > > processRuntimeException( Exception e ) {
        if ( LOGGER.isErrorEnabled() ) {
            LOGGER.error( "Ocurrió un error severo en el JSE-API" );
            LOGGER.error( String.format( "API Causa %s", e.getMessage() ) );
            LOGGER.error( String.format( "Mayor Detalle %s", e.getLocalizedMessage() != null ? e.getMessage() : "No existe mayor detalle" ) );
        }
